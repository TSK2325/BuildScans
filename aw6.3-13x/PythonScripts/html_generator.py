...
def get_row_tag_for_cp_info(idx: int, dms_node_name: str, cp_name: str, lnx_build_err_files: list, wntx_build_err_files: list, is_wntx_lnx_err_files_equals: bool, is_triage_by_devops_member: bool) -> str:
    cp_name = cp_name.strip()
    link = f"http://devops/self-service/dms/developer-pipeline?cp={cp_name}"

    lnxFlag = False
    if len(lnx_build_err_files) > 0:
        lnxFlag = True

    wntxflag = False
    if len(wntx_build_err_files) > 0:
        wntxflag = True
    ...
    row_tag = f"""
            <tr>
                ...
                <td>
                    {str(lnx_build_err_files)}
                </td>
                <td>
                    {str(wntxflag)}
                </td>
                <td>
                    {str(wntx_build_err_files)}
                </td>
                <td>
...
<!DOCTYPE html>
    <html>
        ...
        for idx, cp in enumerate(cps_meta_data):
            print(cp)
            windows_build_err_files = cps_meta_data[cp]["windows_build_err_file"]
            linux_build_err_files = cps_meta_data[cp]["linux_build_err_file"]
            is_same_wntx_lnx_build_err = cps_meta_data[cp]["is_same_wntx_lnx_build_err"]
            is_triage = cps_meta_data[cp]["is_triage"]
            row_tag = get_row_tag_for_cp_info(
                idx+1 , dms_node_name, cp, linux_build_err_files, windows_build_err_files, is_same_wntx_lnx_build_err, is_triage)
        ...
        with open(out_dir / f"{dms_node_name}.html", 'w') as file:
